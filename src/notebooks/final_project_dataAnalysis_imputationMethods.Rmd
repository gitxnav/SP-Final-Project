...
ckd_knn_basic <- VIM::kNN(
  data = ckd_clean %>% select(-status_binary),  # Remove derived variable
  k = 5,
...
ckd_for_imputation <- ckd_clean %>% select(-status_binary)

# Stage 1: Impute variables with low missingness first
ckd_stage1 <- ckd_for_imputation
...
ckd_knn_advanced <- VIM::kNN(
  ...
  imp_var = TRUE,                           # Keep imputation indicators
  trace = TRUE,
  useImputedDist = TRUE
)
...
# Add back status_binary
ckd_imputed$status_binary <- ckd_clean$status_binary
...
set.seed(123)
validation_data <- ckd_for_imputation

# Select 10% of values to make missing (MAR - Missing at Random)
n_to_missing <- round(nrow(validation_data) * ncol(validation_data) * 0.10)
missing_indices <- sample(1:(nrow(validation_data) * ncol(validation_data)), 
                          n_to_missing)

# Store original values
original_values <- matrix(NA, nrow = nrow(validation_data), 
                          ncol = ncol(validation_data))
for(idx in missing_indices) {
  row_idx <- ((idx - 1) %% nrow(validation_data)) + 1
  col_idx <- floor((idx - 1) / nrow(validation_data)) + 1
  original_values[row_idx, col_idx] <- validation_data[row_idx, col_idx]
  validation_data[row_idx, col_idx] <- NA
}
...
for(col in 1:ncol(validation_data)) {
  col_name <- colnames(validation_data)[col]
...
# Perform MICE imputation for comparison
if(require(mice)) {
  cat("Performing MICE imputation...\n")
  
  mice_start <- Sys.time()
  mice_imputed <- mice(ckd_for_imputation, 
                       m = 5, 
                       maxit = 10, 
  ...
  # Extract first imputed dataset
  ckd_mice <- complete(mice_imputed, 1)
  mice_end <- Sys.time()
  
  cat(sprintf("MICE imputation completed in %.2f seconds\n", 
              mice_end - mice_start))
  
  # Compare missing values
  cat("\nMissing Values Comparison:\n")
  comparison <- data.frame(
    Variable = names(ckd_for_imputation),
    Original_NA = colSums(is.na(ckd_for_imputation)),
    kNN_NA = colSums(is.na(ckd_imputed)),
    MICE_NA = colSums(is.na(ckd_mice))
  )
  ...
  for(var in c("age", "sc", "hemo")) {
    if(is.numeric(ckd_for_imputation[[var]])) {
      # Calculate means
      orig_mean <- mean(ckd_for_imputation[[var]], na.rm = TRUE)
      knn_mean <- mean(ckd_imputed[[var]], na.rm = TRUE)
      mice_mean <- mean(ckd_mice[[var]], na.rm = TRUE)
  ...
  comparison_plot_data <- data.frame(
    ...
    Age = c(ckd_imputed$age, ckd_mice$age),
    Creatinine = c(ckd_imputed$sc, ckd_mice$sc),
    Hemoglobin = c(ckd_imputed$hemo, ckd_mice$hemo)
  )
...
mf_input <- ckd_clean |>
  # Select key predictors and the target variable
  # Adjust this list based on your analysis focus
  select(age, bp, sc, hemo, bu, htn, dm, status) |>
  # Ensure categorical variables are factors
  mutate(across(c(htn, dm, status), as.factor)) |>
  as.data.frame() # Convert to base data.frame as recommended
...
plot_dist_comparison <- function(orig_data, knn_data, mf_data, var_name, method_names = c("Original (with NAs)", "kNN Imputed", "missForest Imputed")) {
...
model_original <- glm(status ~ age + sc + hemo + htn, 
                      data = ckd_clean, family = binomial, na.action = na.omit)
model_knn <- glm(status ~ age + sc + hemo + htn, 
                 data = ckd_knn_basic, family = binomial)
model_missForest <- glm(status ~ age + sc + hemo + htn, 
...
extract_context <- function(row_idx, col_names) {
  ...
  context_vars <- c("status", "age", "sc", "hemo", "htn", "dm")
...
if(exists("ckd_mice")) {
  missing_df$MICE_value <- mapply(extract_imputed_values,
                                 missing_df$row, missing_df$column,
                                 MoreArgs = list(dataset = ckd_mice))
}
...
```{r}
# Analyze numeric variables separately
numeric_vars <- c("age", "bp", "sc", "hemo", "bu", "pcv")

numeric_comparison <- comparison_table %>%
  filter(variable %in% numeric_vars) %>%
  mutate(
    kNN_value = as.numeric(kNN_value),
    missForest_value = as.numeric(missForest_value),
    # Calculate differences
    difference = kNN_value - missForest_value,
    abs_difference = abs(difference),
    # Calculate percentage difference relative to mean of imputed values
    pct_difference = (difference / ((kNN_value + missForest_value)/2)) * 100
  )

# Summary statistics by variable
numeric_summary <- numeric_comparison %>%
  group_by(variable) %>%
  summarise(
    n_missing = n(),
    mean_kNN = mean(kNN_value, na.rm = TRUE),
    mean_missForest = mean(missForest_value, na.rm = TRUE),
    ...
    agreement_rate = sum(abs_difference < 0.1) / n()  # Values within 0.1 of each other
  )

print("Summary of Numeric Variable Imputations:")
print(numeric_summary)

# Visualize the differences
ggplot(numeric_comparison, aes(x = difference, fill = variable)) +
  geom_histogram(bins = 30, alpha = 0.7) +
  facet_wrap(~ variable, scales = "free") +
  geom_vline(xintercept = 0, linetype = "dashed", color = "red") +
  labs(title = "Difference Between kNN and missForest Imputations (kNN - missForest)",
       x = "Difference",
       y = "Count") +
  theme_minimal()
```
...
categorical_vars <- c("htn", "dm", "cad", "appet", "pe", "ane", "status")

categorical_comparison <- comparison_table %>%
  filter(variable %in% categorical_vars) %>%
  mutate(
    agreement = kNN_value == missForest_value,
    kNN_value = as.character(kNN_value),
    missForest_value = as.character(missForest_value)
  )
...
for(var in categorical_vars) {
  ...
  var_data <- categorical_comparison %>%
    filter(variable == var)
  
  if(nrow(var_data) > 0) {
    contingency <- table(
      kNN = factor(var_data$kNN_value, levels = unique(c(var_data$kNN_value, var_data$missForest_value))),
      missForest = factor(var_data$missForest_value, levels = unique(c(var_data$kNN_value, var_data$missForest_value)))
    )
    
    print(contingency)
    
    # Calculate agreement statistics
    agreement_rate <- sum(diag(contingency)) / sum(contingency)
    cat(sprintf("Agreement rate: %.2f%%\n", agreement_rate * 100))
    
    # Cohen's Kappa if binary
    if(nrow(contingency) == 2 && ncol(contingency) == 2) {
      library(psych)
      kappa_result <- cohen.kappa(contingency)
      cat(sprintf("Cohen's Kappa: %.3f (agreement: %s)\n", 
                  kappa_result$kappa, 
                  case_when(
                    kappa_result$kappa < 0 ~ "No agreement",
                    kappa_result$kappa < 0.2 ~ "Slight agreement",
                    kappa_result$kappa < 0.4 ~ "Fair agreement",
                    kappa_result$kappa < 0.6 ~ "Moderate agreement",
                    kappa_result$kappa < 0.8 ~ "Substantial agreement",
...
# Overall agreement for categorical variables
categorical_summary <- categorical_comparison %>%
  group_by(variable) %>%
  summarise(
    n_missing = n(),
    n_agreement = sum(agreement),
    agreement_rate = n_agreement / n(),
    most_common_kNN = names(sort(table(kNN_value), decreasing = TRUE))[1],
    most_common_missForest = names(sort(table(missForest_value), decreasing = TRUE))[1]
  )

print("Categorical Variables Agreement Summary:")
...
```{r}
# Analyze imputation patterns by patient CKD status
imputation_by_status <- comparison_table %>%
  mutate(
    kNN_value = as.character(kNN_value),
    missForest_value = as.character(missForest_value),
    `Context status` = factor(`Context status`, levels = c("ckd", "notckd"))
  ) %>%
  filter(!is.na(`Context status`)) %>%
  group_by(variable, `Context status`) %>%
  summarise(
    count = n(),
    # For numeric-like variables, calculate means if possible
    mean_kNN = if(all(grepl("^[0-9.-]+$", kNN_value))) 
      mean(as.numeric(kNN_value), na.rm = TRUE) else NA,
    mean_missForest = if(all(grepl("^[0-9.-]+$", missForest_value))) 
      mean(as.numeric(missForest_value), na.rm = TRUE) else NA,
    # For categorical, show distribution
    kNN_distribution = if(!all(grepl("^[0-9.-]+$", kNN_value))) 
      paste(names(sort(table(kNN_value), decreasing = TRUE)[1:2]), collapse = ", ") else NA,
    missForest_distribution = if(!all(grepl("^[0-9.-]+$", missForest_value))) 
      paste(names(sort(table(missForest_value), decreasing = TRUE)[1:2]), collapse = ", ") else NA,
    .groups = 'drop'
  )

# Create a comparison table
status_comparison <- imputation_by_status %>%
  select(variable, `Context status`, count, 
         starts_with("mean"), starts_with("distribution"))

print("Imputation Patterns by CKD Status:")
print(status_comparison, n = 30)

# Visualize key biomarker imputations by status
key_biomarkers <- c("sc", "hemo", "bu")

biomarker_comparison <- comparison_table %>%
  filter(variable %in% key_biomarkers) %>%
  mutate(
    kNN_value = as.numeric(kNN_value),
    missForest_value = as.numeric(missForest_value),
    `Context status` = factor(`Context status`, levels = c("ckd", "notckd"))
...
ggplot(biomarker_comparison, aes(x = `Context status`, y = kNN_value, fill = "kNN")) +
  geom_boxplot(alpha = 0.5, position = position_dodge(width = 0.8)) +
  geom_boxplot(aes(y = missForest_value, fill = "missForest"), 
               alpha = 0.5, position = position_dodge(width = 0.8)) +
  facet_wrap(~ variable, scales = "free_y") +
  labs(title = "Imputed Biomarker Values by CKD Status and Method",
       x = "CKD Status",
       y = "Imputed Value",
       fill = "Imputation Method") +
  theme_minimal() +
  scale_fill_manual(values = c("kNN" = "blue", "missForest" = "red"))
```

```{r}
# Analyze systematic differences between methods
pattern_analysis <- function(data, variable_name) {
  var_data <- data %>% filter(variable == variable_name)
  
  if(nrow(var_data) == 0) return(NULL)
  
  # Check if numeric
  if(all(grepl("^[0-9.-]+$", var_data$kNN_value)) && 
     all(grepl("^[0-9.-]+$", var_data$missForest_value))) {
    ...
    t_test <- t.test(var_data$difference)
    
    result <- data.frame(
      Variable = variable_name,
      Type = "Numeric",
      Mean_Difference = mean(var_data$difference),
      CI_Lower = t_test$conf.int[1],
      CI_Upper = t_test$conf.int[2],
      P_Value = t_test$p.value,
      Systematic_Bias = ifelse(t_test$p.value < 0.05, "Yes", "No"),
      Bias_Direction = ifelse(t_test$p.value < 0.05, 
                              ifelse(mean(var_data$difference) > 0, "kNN higher", "missForest higher"),
                              "No significant bias")
    )
    
  } else {
    # Categorical analysis
    contingency <- table(var_data$kNN_value, var_data$missForest_value)
    
    result <- data.frame(
      Variable = variable_name,
      Type = "Categorical",
      Agreement_Rate = sum(diag(contingency)) / sum(contingency),
      Most_Common_kNN = names(which.max(table(var_data$kNN_value))),
      Most_Common_missForest = names(which.max(table(var_data$missForest_value))),
      Systematic_Bias = ifelse(names(which.max(table(var_data$kNN_value))) == 
                               names(which.max(table(var_data$missForest_value))),
                               "No", "Yes")
    )
  }
...
# Run pattern analysis for all variables with missing values
variables_with_missing <- unique(comparison_table$variable)
pattern_results <- lapply(variables_with_missing, function(v) {
  pattern_analysis(comparison_table, v)
})

pattern_summary <- do.call(rbind, pattern_results) %>%
  arrange(Type, desc(ifelse(Type == "Numeric", abs(Mean_Difference), 1-Agreement_Rate)))

print("Systematic Differences Between Imputation Methods:")
print(pattern_summary)

# Create a visualization of systematic biases
if(any(pattern_summary$Type == "Numeric")) {
  numeric_patterns <- pattern_summary %>% filter(Type == "Numeric")
  
  ggplot(numeric_patterns, aes(x = reorder(Variable, Mean_Difference), 
                               y = Mean_Difference,
                               fill = Systematic_Bias)) +
    geom_bar(stat = "identity") +
    geom_errorbar(aes(ymin = CI_Lower, ymax = CI_Upper), width = 0.2) +
    geom_hline(yintercept = 0, linetype = "dashed", color = "red") +
    coord_flip() +
    labs(title = "Systematic Differences in Numeric Variable Imputation",
         subtitle = "Positive values = kNN imputes higher values",
         x = "Variable",
         y = "Mean Difference (kNN - missForest)") +
    theme_minimal()
}
```
